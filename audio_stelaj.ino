#define defaultMode 4 //режим при запуске платы: 0 - без анализатора, 1 - бегающие одинаковые столбы радуги, 2 - бегающие разны столбы разной радуги, 3 - изменяющие яркость радужные столбы
#define USE_WIFI 0 //использовать ли MQTT и WIFI
#define MOVING_POINT 0  //влючить эффект падающей точки
#define DEBUG_ENABLE 0 //выводить в порт: 0 - ничего, 1 - сразу после Фурье, 2 - после масштабирования под ленту, 3 - падающая точка
...
#define colorPoint 255  //цвет точки 0 - 255 (крайние точки - красный, дальше хз как там)
...
#define powerKoef 0 // коэфф для повышения чувствительности с ростом частот

//настройки радуги
#define timeToFallColums 100 //время падения столбца
...
//настройки режима вспышек
#define lowBrightness 70  //минимальное значение яркости 
#define lowColor 255  //значение минимального 0 - 255 (крайние точки - красный, дальше хз как там)
int timeToFallOutbreaks = 5;  //время погасания вспышки
int bowSpeed = 25;  //скоротсь изменения радуги
int bowStep = 20; //шаг радуги между светодиодами
int levelColors[] = {0, 11, 42, 70, 112, 133, 146, 180};
...
int thisMode = defaultMode;  //переменная выбора режима
...
#endif
  if (micros() - microseconds > samplingPeriod) {
    microseconds += samplingPeriod;
    switch (thisMode) {
      case 1:
        sampling(); //слушаю микрик и считаю Фурье
        calculatingColums();  //считаю нужное количество столбцов и добавляю плавность
        animation();  //вывожу на каждый светодиод свое значение
        fallPoint();  //падающая точка, если включенна
...
void sampling() {
  ...
  */
  ...
  //складываю значения всех столбиков
  int steps = (samples) / (xres + 2); //(2 - два крайних значения, их не учитываю, показывают погоду)
  int w = 0;
  for (int i = steps; i <= samples - steps; i += steps)  {
    data_avgs[w] = 0;
    for (int k = 0 ; k < steps ; k++) {
      data_avgs[w] = data_avgs[w] + vReal[i + k];
    }
    data_avgs[w] = data_avgs[w] / steps;
    w++;
  }
  //симуляция звука
  if (!digitalRead(0)) {
...
void calculatingColums() {
  for (int i = 0; i < xres; i++)
  {
    ...
    if (millis() > displayTime[i] && peaks[i] != 0) { //плавное падение
      displayTime[i] = millis() + timeToFallColums;
...
void calculatingOutbreaks() {
  for (int i = 0; i < xres; i++)
  {
    ...
    //Serial.println(smoothValue[0]);
    data_avgs[i] = constrain(data_avgs[i], noiseFilter, micSensetive - (i * powerKoef) + smoothValue[i]); //ограничиваю массив
    data_avgs[i] = map(data_avgs[i], noiseFilter, micSensetive - (i * powerKoef) + smoothValue[i], 0, 255);  //маштабирую массив под размеры матрицы
    yvalue = data_avgs[i];
    if (millis() > displayTime[i] && peaks[i] != 0) { //плавное падение
      displayTime[i] = millis() + timeToFallOutbreaks;
      peaks[i] = peaks[i] - 1;    // decay by one light
      //Serial.println(peaks[i]);
    }
    if (yvalue >= peaks[i]) {
...
void animation() {
  switch (thisMode) {
    ...
    case 2:
      ...
      break;
    case 3:
      for (int k = 0; k < xres; k++ ) {  //для каждой строчки
        if (displayvalue[k] > lowBrightness) {
          color = (counter * yres) + ((256 / xres) * (k + bowStep));
          for (int z = 0; z < yres; z++) {
            leds[(k * yres) + z] = CHSV(color, 255, displayvalue[k]);
          }
        } else {
          for (int v = 0; v < yres; v++ ) {
            leds[(k * yres) + v] = CHSV(lowColor, 255, lowBrightness); //проверяю длину столбца и заливаю радугой
          }
        }
      }
      if (millis() > rainbowTimer) {
        counter++;
        rainbowTimer = millis() + bowSpeed;
      }
      break;
    case 4:
      for (int k = 0; k < xres; k++ ) {  //для каждой строчки
        if (displayvalue[k] > lowBrightness) {
          for (int z = 0; z < yres; z++) {
            leds[(k * yres) + z] = CHSV(levelColors[k], 255, displayvalue[k]);
          }
        } else {
          for (int v = 0; v < yres; v++ ) {
            leds[(k * yres) + v] = CHSV(lowColor, 255, lowBrightness); //проверяю длину столбца и заливаю радугой
...
void fallPoint () { //бегущая точка
  ...
  for (int j = 0; j < xres; j = j + 2) {
    leds[(j * yres) + maxPoint[j]] = CHSV(colorPoint, 255, 255);
  }
  for (int h = 1; h < xres; h = h + 2) {
    leds[(h * yres) + yres - maxPoint[h] - 1] = CHSV(colorPoint, 255, 255);
  }
#endif
#if (DEBUG_ENABLE == 3)
